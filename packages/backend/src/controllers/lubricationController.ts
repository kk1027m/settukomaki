...
export const createLubricationPoint = async (req: AuthRequest, res: Response, next: any) => {
  try {
    ...
    const result = await query(
      `INSERT INTO lubrication_points (machine_name, unit_name, location, cycle_days, description, created_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [machine_name, unit_name || null, location, cycle_days, description || null, req.user?.id]
    );
    ...
    await createNotificationTopic(
      '給油箇所が追加されました',
      `${machine_name}${unit_name ? ' - ' + unit_name : ''} - ${location} の給油箇所が追加されました。`,
      req.user?.id || 1
    );
...
export const updateLubricationPoint = async (req: AuthRequest, res: Response, next: any) => {
  try {
    ...
    await createNotificationTopic(
      '給油箇所が編集されました',
      `${updatedPoint.machine_name}${updatedPoint.unit_name ? ' - ' + updatedPoint.unit_name : ''} - ${updatedPoint.location} の給油箇所が編集されました。`,
      req.user?.id || 1
    );
...
export const performLubrication = async (req: AuthRequest, res: Response, next: any) => {
  try {
    ...
    await createNotificationTopic(
      '給油が完了しました',
      `${point.machine_name}${point.unit_name ? ' - ' + point.unit_name : ''} - ${point.location} の給油が完了しました。`,
      req.user?.id || 1
    );
