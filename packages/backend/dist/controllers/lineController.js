...
const checkAndSendNotifications = async () => {
    ...
    const overdueLubricationResult = await (0, connection_1.query)(`SELECT DISTINCT ON (lp.id)
     ...
     ORDER BY lp.id, lr.performed_at DESC`, [today.toISOString().split('T')[0]]);
    ...
    const overdueReplacementResult = await (0, connection_1.query)(`SELECT DISTINCT ON (rs.id)
     ...
     ORDER BY rs.id, rr.replaced_at DESC`, [today.toISOString().split('T')[0]]);
